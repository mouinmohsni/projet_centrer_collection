...
class FichePaie{
    ...
    static async create(id_user, id_date, salaire_base, prime = 0, retenue = 0) {
        ...
        try {
            ...
            return new FichePaie(result.insertId, id_user, id_date, salaire_base, prime, retenue, net_paye);
    ...
    static async findById(id_fiche) {
        ...
        const row = rows[0];
        return new FichePaie(row.id_fiche, row.id_date, row.id_date, row.salaire_base, row.prime, row.deductions, row.salaire_net);
    ...
    static async getByUser(id_user) {
        try {
            ...
            return rows.map(row => new FichePaie(row.id_fiche, row.id_user, row.id_date, row.salaire_base, row.prime, row.retenue, row.net_paye));
        }catch (e) {
            console.error('Erreur DB dans paie :', e);
            throw e; // pour que le contrôleur connaisse le problème
        }
